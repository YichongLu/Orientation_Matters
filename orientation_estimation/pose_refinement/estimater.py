...
class FoundationPose:
  def __init__(self, model_pts, model_normals, symmetry_tfs=None, mesh=None, scorer:ScorePredictor=None, refiner:PoseRefinePredictor=None, glctx=None, debug=0, debug_dir='/home/bowen/debug/novel_pose_debug/', bg_color='white', object_level=False, rgb_only=False, crop_ratio=1.0):
    self.gt_pose = None
  ...
  def register(self, K, rgb, depth, ob_mask, ob_id=None, glctx=None, iteration=5):
    ...
    refiner_debug_dir = f'{self.debug_dir}/refiner'
    os.makedirs(refiner_debug_dir, exist_ok=True)
    poses, vis = self.refiner.predict(refiner_debug_dir=refiner_debug_dir, mesh=self.mesh, mesh_tensors=self.mesh_tensors, rgb=rgb, depth=depth, K=K, ob_in_cams=poses.data.cpu().numpy(), normal_map=normal_map, xyz_map=xyz_map, glctx=self.glctx, mesh_diameter=self.diameter, iteration=iteration, get_vis=self.debug>=2, bg_color=self.bg_color, object_level=self.object_level, rgb_only=self.rgb_only, crop_ratio=self.crop_ratio)
    
    if vis is not None:
      imageio.imwrite(f'{self.debug_dir}/vis_refiner.png', vis)

    scores, vis = self.scorer.predict(mesh=self.mesh, rgb=rgb, depth=depth, K=K, ob_in_cams=poses.data.cpu().numpy(), normal_map=normal_map, mesh_tensors=self.mesh_tensors, glctx=self.glctx, mesh_diameter=self.diameter, get_vis=self.debug>=2, object_level=self.object_level, rgb_only=self.rgb_only, crop_ratio=self.crop_ratio)
    if vis is not None:
