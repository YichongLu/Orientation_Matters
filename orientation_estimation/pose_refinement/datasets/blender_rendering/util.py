...
def get_archimedean_spiral(sphere_radius, num_steps=250):
    ...
    a = 8
    r = sphere_radius
    i_list = np.linspace(a / 2, a, num_steps)
    translations = []
    ...
    for i in i_list:
        rt = r * (0.9+ abs(math.sin(-i *  math.pi)) * 0.1)
        theta =  math.pi  * 16 / 15 / 2 + math.pi  * 1/16 * (i - a/2) / a
        cos_v = 0 if abs(math.cos(-i *  math.pi)) < 0.1 else math.cos(-i *  math.pi)
        sin_v = 0 if abs(math.sin(-i *  math.pi)) < 0.25 else math.sin(-i *  math.pi)
        x = rt * math.sin(theta) * cos_v
        z = rt * math.sin(-theta + math.pi) * sin_v#* math.sin(-i)
        y = rt * - math.cos(theta)
