...
class YcbineoatReader:
  ...
  def get_depth(self,i):
    depth_file = self.color_files[i].replace('rgb','depth').replace('png','exr')
    if os.path.exists(depth_file):
      with OpenEXR.File(depth_file) as infile:
        header = infile.header()
        print(f"type={header['type']}")
        print(f"compression={header['compression']}")
        depth = np.array(infile.channels()["V"].pixels,dtype=np.float64)
    else:
    ...
    zmin = depth.min()
    zmax = depth.max()
    depth_vis = depth_to_vis(depth, zmin=zmin, zmax=zmax, inverse=False, mode='rgb')
    return depth
