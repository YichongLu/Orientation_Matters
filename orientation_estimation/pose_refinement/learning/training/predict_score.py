...
def vis_batch_data_scores(pose_data, ids, scores, pad_margin=5):
  ...
  for id in ids[:10]:
    ...
    # debug depth in gray mode
    depthA_vis_gray = depth_to_vis(pose_data.depthAs[id].data.cpu().numpy().reshape(H,W), zmin=zmin, zmax=zmax, inverse=False, mode='gray')
    depthB_vis_gray = depth_to_vis(pose_data.depthBs[id].data.cpu().numpy().reshape(H,W), zmin=zmin, zmax=zmax, inverse=False, mode='gray')
...
@torch.no_grad()
def make_crop_data_batch(render_size, ob_in_cams, mesh, rgb, depth, K, crop_ratio, normal_map=None, mesh_diameter=None, glctx=None, mesh_tensors=None, dataset:TripletH5Dataset=None, cfg=None, bg_color='white', object_level=False, rgb_only=False):
  ...
  if object_level:
    if isinstance(depth, torch.Tensor):
      depthBs = torch.as_tensor(cv2.resize(depth.data.cpu().numpy(), (160, 160), interpolation=cv2.INTER_NEAREST), dtype=torch.float, device='cuda')[None,None].expand(B,-1,-1,-1)
    elif isinstance(depth, np.ndarray):
      depthBs = torch.as_tensor(cv2.resize(depth, (160, 160), interpolation=cv2.INTER_NEAREST), dtype=torch.float, device='cuda')[None,None].expand(B,-1,-1,-1)
    else:
  ...
  if rgb_only:
    depthAs = torch.zeros_like(depthAs)
    depthBs = torch.zeros_like(depthBs)
...
class ScorePredictor:
  def __init__(self, amp=True, ckpt_root_dir=None):
    self.amp = amp
  ...
  def predict(self, rgb, depth, K, ob_in_cams, normal_map=None, get_vis=False, mesh=None, mesh_tensors=None, glctx=None, mesh_diameter=None, bg_color='white', object_level=False, rgb_only=False, crop_ratio=1.4):
    '''
    @rgb: np array (H,W,3)
    '''
    ...
    pose_data = make_crop_data_batch(self.cfg.input_resize, ob_in_cams, mesh, rgb, depth, K, crop_ratio=crop_ratio, glctx=glctx, mesh_tensors=mesh_tensors, dataset=self.dataset, cfg=self.cfg, mesh_diameter=mesh_diameter, bg_color=bg_color, object_level=object_level, rgb_only=rgb_only)
