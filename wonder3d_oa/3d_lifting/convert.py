...
class Converter(nn.Module):
    def __init__(self, opt: Options, test_path: str):
        super().__init__()
        ...
        self.gaussians = self.gs_renderer.load_ply(test_path).to(self.device)
...
if opt.input_dir is not None:
    input_paths = [os.path.join(opt.input_dir, f) for f in os.listdir(opt.input_dir) if f.endswith('.ply')]
else:
...
for input_path in input_paths:
    ...
    converter = Converter(opt, input_path).cuda()
    converter.fit_nerf()
    converter.fit_mesh()
    converter.fit_mesh_uv()
    converter.export_mesh(input_path.replace('.ply', '.glb'))
