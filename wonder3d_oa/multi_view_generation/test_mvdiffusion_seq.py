...
def log_validation(dataloader, pipeline, cfg: TestConfig, weight_dtype, name, save_dir):
    ...
    for i, batch in tqdm(enumerate(dataloader)):
        ...
        task_embeddings = batch['color_task_embeddings']
        ...
        if cfg.cam_type == 'ortho':
            cam_type_emb = torch.cat([torch.tensor([0, 1]).expand(num_views, -1).unsqueeze(0)], dim=0)
            camera_embeddings = torch.cat([camera_embeddings, cam_type_emb], dim=-1)
        elif cfg.cam_type == 'persp':
            cam_type_emb = torch.cat([torch.tensor([1, 0]).expand(num_views, -1).unsqueeze(0)], dim=0)
            camera_embeddings = torch.cat([camera_embeddings, cam_type_emb], dim=-1)
        else:
            print("Do not load camera type.")
...
def log_validation_joint(dataloader, pipeline, cfg: TestConfig, weight_dtype, name, save_dir):
    ...
    for i, batch in tqdm(enumerate(dataloader)):
        ...
        if cfg.cam_type == 'ortho':
            cam_type_emb = torch.cat([torch.tensor([0, 1]).expand(num_views, -1).unsqueeze(0)]*2, dim=0)
            camera_embeddings = torch.cat([camera_embeddings, cam_type_emb], dim=-1)
        elif cfg.cam_type == 'persp':
            cam_type_emb = torch.cat([torch.tensor([1, 0]).expand(num_views, -1).unsqueeze(0)]*2, dim=0)
            camera_embeddings = torch.cat([camera_embeddings, cam_type_emb], dim=-1)
        else:
            print("Do not load camera type.")
...
def load_wonder3d_pipeline(cfg):
    ...
    pipeline = MVDiffusionImagePipeline.from_pretrained(cfg.pretrained_model_name_or_path, unet=unet, torch_dtype=weight_dtype, **cfg.pipe_kwargs)
    if cfg.load_lora:
        # load lora weights
        lora_scale = cfg.lora_alpha / cfg.rank
        pipeline.load_lora_weights(cfg.lora_output_dir, weight_name="pytorch_lora_weights.safetensors", adapter_name="default")
        pipeline.fuse_lora(components=['unet'], lora_scale=lora_scale)
...
if __name__ == '__main__':
    ...
    elif cfg.num_views == 7:
        VIEWS = ['front', 'front_right', 'right', 'back', 'left', 'front_left', 'random']
    else:
