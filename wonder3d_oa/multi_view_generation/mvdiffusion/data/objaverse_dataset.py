...
class ObjaverseDataset(Dataset):
    def __init__(self,
        ...
        with open(condition_dir, 'rb') as zip_file:
            zip_data = zip_file.read()
        zip_memory = io.BytesIO(zip_data)
        with zipfile.ZipFile(zip_memory, 'r') as zip_ref:
            self.condition_dataset = {file: zip_ref.read(file) for file in zip_ref.namelist()} 
        ...
        self.all_objects_random = list(self.all_objects)
    ...
    def load_image(self, root_dir, file_name, bg_color, alpha, return_type='np'):
        ...
        elif root_dir == self.condition_dir:
            image_bytes = io.BytesIO(self.condition_dataset[file_name])
        else:
    ...
    def __getitem_mix__(self, index, debug_object=None):
        ...
        img_tensors_in = [ self.load_image(self.condition_dir, os.path.join(str(self.condition_dir).split('/')[-1].split('.')[0], object_name[:self.subscene_tag], object_name, "rgb_%03d_%s.%s" % (set_idx, cond_view, self.suffix)), bg_color, cond_alpha, return_type='pt').permute(2, 0, 1)
                ] * self.num_views
        img_tensors_out = []
