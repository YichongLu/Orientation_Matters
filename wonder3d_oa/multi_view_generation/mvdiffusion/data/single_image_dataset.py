...
def add_margin(pil_img, color=1, size=256):
...
class SingleImageDataset(Dataset):
    def __init__(self,
        ...
        elif self.num_views == 7:
            self.view_types  = ['front', 'front_right', 'right', 'back', 'left', 'front_left', 'random']
        else:
    ...
    def load_image(self, img_path, bg_color, return_type='np', Imagefile=None):
        ...
        if self.crop_size!=-1:
            if self.bg_color == 'white':
                color_int8 = (255, 255, 255)
            elif self.bg_color == 'black':
                color_int8 = (0,0,0)
            else:
                raise NotImplementedError
            if np.asarray(image_input).shape[2] == 3:
                coords = np.where(np.asarray(image_input)[:, :, 2] < 255)
                min_x, max_x = np.min(coords[0]), np.max(coords[0])
                min_y, max_y = np.min(coords[1]), np.max(coords[1])
            elif np.asarray(image_input).shape[2] == 4:
                alpha_np = np.asarray(image_input)[:, :, 3]
                # coords = np.stack(np.nonzero(alpha_np), 1)[:, (1, 0)]
                coords = np.where(alpha_np > 0)
                blank_coords = np.where(alpha_np == 0)
                image_input_np = np.array(image_input)
                for x, y in zip(blank_coords[0], blank_coords[1]):
                    image_input_np[x, y, :3] = color_int8
                image_input = Image.fromarray(image_input_np)
                min_x, max_x = np.min(coords[0]), np.max(coords[0])
                min_y, max_y = np.min(coords[1]), np.max(coords[1])
            else:
                raise NotImplementedError
            
            ref_img_ = image_input.crop((min_y, min_x, max_y, max_x))
            h, w = ref_img_.height, ref_img_.width
            scale = self.crop_size / max(h, w)
            h_, w_ = int(scale * h), int(scale * w)
            ref_img_ = ref_img_.resize((w_, h_))

            image_input = add_margin(ref_img_, size=image_size, color=color_int8)
        else:
    ...
    def __getitem__(self, index):
        ...
        for view, tgt_w2c in zip(self.view_types, tgt_w2cs):
            ...
            if view == 'random':
                elevation = torch.zeros(elevation.shape)
                azimuth = torch.zeros(azimuth.shape)
